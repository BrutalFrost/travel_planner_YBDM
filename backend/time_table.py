...
def calculate_minutes_remaining_depart(df):
    ...
    df["departure_datetime"] = pd.to_datetime(df["date"].astype(str) + " " + df["time"].astype(str))
    ...
    df["Directed To"] = df["direction"].str.replace(r"\s*\(Göteborg kn\)", "", regex=True) if "direction" in df else "Unknown"
...
def calculate_minutes_remaining_arrival(df):
    ...
    current_time = datetime.now()
    df["arrival_datetime"] = pd.to_datetime(df["date"].astype(str) + " " + df["time"].astype(str))
    df["minutes_remaining"] = (df["arrival_datetime"] - current_time).dt.total_seconds() // 60
    df = df[df["minutes_remaining"] >= 0].copy()

    df["name"] = df["name"].str.replace(r"^Länstrafik - ", "", regex=True)

    df["From"] = (
        df["direction"].str.replace(r"\s*\(Göteborg kn\)", "", regex=True) if "direction" in df

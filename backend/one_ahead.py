...
def calculate_minutes_remaining(df):
    ...
    df['departure_datetime'] = df['departure_datetime'].dt.tz_localize("Europe/Stockholm", ambiguous="NaT", nonexistent="NaT")
    ...
    df.loc[:, 'direction'] = df['direction'].str.replace(r'\s*\(Göteborg kn\)', '', regex=True)
...
if df_after_one_hour.empty:
    ...
    print(df_after_one_hour.to_string(index=False))  # Remove default index for a cleaner output

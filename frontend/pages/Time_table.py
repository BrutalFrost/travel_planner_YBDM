...
from backend.time_table import ResRobot , fetch_timetable , calculate_minutes_remaining
...
if location_name_one_hour:  
    ...
    if ext_id_one_hour is not None:  
        ...
        if df_one_hour.empty:
            st.warning(f"No departures scheduled one hour ahead for '{location_name_one_hour}'. Try another stop.")
        else:
